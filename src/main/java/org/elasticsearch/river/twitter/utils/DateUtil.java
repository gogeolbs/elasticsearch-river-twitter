...
public final class DateUtil {
    ...
    private static final HashMap<String, String> DATE_FORMAT_REGEXPS = new HashMap<String, String>() {
		private static final long serialVersionUID = 8244050687250330027L;
		{
		    ...
		    put("^[A-Z a-z]{3},\\s\\d{1,2}\\s[A-Z a-z]{3}\\s\\d{4}\\s\\d{1,2}:\\d{1,2}:\\d{1,2}\\s[+ -]\\d{4}$", "EEE, d MMM yyyy HH:mm:ss Z");
		    put("^\\d{5}.[A-Z a-z]{4,9}.\\d{1,2}\\s[A-Z]{2}\\s\\d{1,2}:\\d{1,2}\\s[A-Z]{2}$", "yyyyy.MMMMM.dd GGG hh:mm aaa");
		    put("^\\d{1,2}:\\d{1,2}\\s[A-Z]{2},\\s[A-Z]{3}$", "K:mm a, z");
		    put("^\\d{1,2}\\so'clock\\s[A-Z]{2},\\s[A-Z a-z]{1,}$", "hh 'o''clock' a, zzzz");
		    put("^\\d{1,2}:\\d{1,2}\\s[A-Z]{2}$", "h:mm a");
		    put("^[A-Z a-z]{3}\\s[A-Z a-z]{3}\\s\\d{1,2}\\s\\d{1,2}:\\d{1,2}:\\d{1,2}\\s[+ -]\\d{4}\\s\\d{4}$", "EEE MMM d HH:mm:ss Z yyyy");
		}
	};
